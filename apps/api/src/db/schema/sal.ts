...
// Auto-generated: 2025-12-23T10:39:16.561991
...
export const salalerta = pgTable('sal_alerta', {
  ...
  tipo: text('tipo').notNull(), // VENCIMIENTO|CUOTA_PENDIENTE|OBRA_SIN_PAGO|COMPROMISO_VENCIDO|RENDICION|OTRO
  nivel: text('nivel').notNull().default('INFO'), // INFO|ATENCION|ALTO|CRITICO
  severidad: text('severidad').notNull().default('MEDIA'),
  origen: text('origen').notNull().default('SISTEMA'),
  mensaje: text('mensaje').notNull(),
  regla_origen: text('regla_origen'),
  fecha_generacion: timestamp('fecha_generacion').notNull().defaultNow(),
  activa: boolean('activa').notNull().default(true),
  atendida: boolean('atendida').notNull().default(false),
  atendida_por_id: uuid('atendida_por_id'),
  fecha_atencion: timestamp('fecha_atencion'),
  ipr_id: uuid('ipr_id'),
