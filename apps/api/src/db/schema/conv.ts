...
// Auto-generated: 2025-12-23T10:39:16.557487
...
export const convcompromiso = pgTable('conv_compromiso', {
  ...
  ipr_id: uuid('ipr_id'),
  tipo_gestion: text('tipo_gestion').notNull(), // GESTION|DOCUMENTO|REUNION|VISITA|TRAMITE|OTRO
  descripcion: text('descripcion').notNull(),
  responsable_id: uuid('responsable_id').notNull(),
  fecha_compromiso: date('fecha_compromiso').notNull().defaultNow(),
  fecha_limite: date('fecha_limite').notNull(),
  prioridad: text('prioridad').notNull().default('NORMAL'),
  estado: text('estado').notNull().default('PENDIENTE'), // PENDIENTE|EN_PROGRESO|COMPLETADO|VERIFICADO|CANCELADO
  porcentaje_avance: integer('porcentaje_avance').notNull().default(0),
  verificado: boolean('verificado').notNull().default(false),
  verificado_por_id: uuid('verificado_por_id'),
  fecha_cumplimiento: timestamp('fecha_cumplimiento'),
