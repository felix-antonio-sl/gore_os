...
**Category Pattern**: each FK → exactly ONE scheme (Categorical Univocity). Enforced by 86 CHECK constraints (`fn_validate_category_scheme`) + 4 triggers. 81 schemes. Only 6 FKs deferred (need new schemes: vehicle, risk prob/status, inventory status, planning instrument). Check before creating: `SELECT DISTINCT scheme FROM ref.category ORDER BY scheme;`
...
~171 endpoints, 400 tests, 32 modules, 22 gate functions. HΩ: 15/15. Parametric: 6/6. Budget classifier: 6/6. Categorical Univocity: 86 CHECKs + 4 triggers (6 deferred). Schema truth: `goreos_ddl_production.sql` (pg_dump). Audit: `docs/GORE_OS_Audit_v3.0.md`. **Gap**: 0 external integrations (ClaveÚnica, PISEE, BIP, SIGFE, CGR).
