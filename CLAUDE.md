...
24. **Admin module** (ADMIN_SISTEMA only): Página única `/admin` con 5 tabs (Usuarios, Divisiones, Configuración, Monitoreo, Auditoría). Las rutas `/admin/*` antiguas son **redirect stubs** → `/admin?tab=` (p.ej. `/admin/divisiones` → `?tab=divisiones`, `/admin/financing-tracks` → `?tab=configuracion`). Formularios de creación en drawers.
...
26. **UX Pruning (C60) — aplicada**: Creación vía drawers inline (`IprCompromisoDrawer`, etc.) y `ROLE_SECTIONS` en `sidebar.tsx` controla `defaultOpen`/visibilidad por rol. Las rutas legacy son **redirect stubs** para bookmarks: 8 páginas `/nuevo` no-IPR → su lista (sólo `/ipr/nuevo` es página real), `/mi-division` → `/dashboard` (absorbida en ModuleMyTeam), `/comite-td` + `/coordinacion/divisiones` → `/coordinacion?tab=`, rutas `/admin/*` viejas → `/admin?tab=`, `/servicios/*` → drawers. De 60 `page.tsx`: **39 navegables + 21 redirect stubs** (las constantes `miDivision`/`comiteTD` en `sidebar.tsx` son legacy, no renderizadas). Matriz rol×página + recomendaciones R1-R7 (implementadas): `docs/GORE_OS_Role_Surface_Spec_v1.0.md`.
