...
import { Plus, Lightbulb, AlertTriangle, ArrowUpRight } from "lucide-react";
...
export default function CoordinacionPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_v: unknown, row: unknown) => {
        const r = row as ARDecision;
        return <Badge variant="outline" className={`text-xs ${STATUS_COLORS[r.status] ?? ""}`}>{r.status_label}</Badge>;
      },
    },
    {
  ...
  return (
    <div className="p-6 space-y-4">
      <PageHeader
        ...
        description="Decisiones y seguimiento con Administrador Regional"
        accentColor="cyan"
        actions={
          canWrite ? (
      ...
      {/* Compact prep summary — clickable links, not informational cards */}
      ...
      {/* Decisions table — the actual working interface */}
      <DataTable
        columns={columns}
